 ...
 */
module.exports.getMyPages = [
    'SELECT', [
        's.setCode setCode',
        // For Page Layout
        's.layout layout',
        'p.page_index page_index',
        's.board_index board_index',
        // For Page Display
        'p.title pagTitle',
        'c.title comTitle',
        'b.title bodTitle'
    ...
    'FROM page_setting s',
    // JOIN
    'INNER JOIN page p ON s.pagCode = p.pagCode',
    'INNER JOIN community c ON s.comCode = c.comCode',
    'INNER JOIN board b ON s.bodCode = b.bodCode',
    // CONDITIONS
    'WHERE s.memCode = ?',
    'AND p.display = 1 AND s.display = 1',
].join(' ');
/**
 * crawler: 세션에서 
 */
module.exports.crawler = [
    ...
    'INNER JOIN community c ON s.comCode = c.comCode',
    'INNER JOIN board b ON s.bodCode = b.bodCode',
    'INNER JOIN update_log l ON s.logCode = l.logCode',
    // CONDITIONS
    'WHERE s.memCode = ?',
    'AND c.title = ?',
    'AND b.title = ?'
