...
Router.post('/auth/login', (req, res)=>{
    ...
    const getMember = new Promise((resolve, reject)=>{
        conn.query( SQL.auth.getMember, [ username ], (error, exist_member)=>{
            if( error ) return reject( {errorCode: 400, error:error, msg: "Get Member Query Error"} );
            if( exist_member.length !== 1 ) return reject({errorCode: 400, error:'Not Found Username'});
        ...
        return new Promise((resolve, reject)=>{
            if( !bcrypt.compareSync( password, member.password ) ){
                return reject({errorCode: 401, error: 'Not Matched Password.'});
            }
            // Set Session
            const user = {
            ...
            conn.query( SQL.getMyPages, [ member.memCode ], (error, myPages)=>{
                ...
                resolve( { user: user, page: JSON.parse(JSON.stringify(myPages)) } );
            });
        });
    }).then((data)=>{
...
Router.post('/auth/register', (req, res)=>{
    ...
    const insertMember = new Promise((resolve, reject)=>{
        // Check Exist User
        conn.query( SQL.auth.getMember, [ username ], (error, exist_member)=>{
            if( error ) reject({errorCode: 400, error:error, msg:"Check Exist Query Error"});
            if( exist_member.length > 0 ) reject({errorCode: 400, error: 'EIXST_MEMBER'});
            // Get Member Count For MemCode
            conn.query( SQL.auth.getMemberCount, (members_error, members)=>{
                if( members_error ) reject( members_error );
